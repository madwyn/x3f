        ...
        double *sum)
{
  ...
  for (row = 0; row < area.rows; row++)
    for (col = 0; col < area.columns; col++)
      for (color = 0; color < colors; color++) {
        double dev = area.data[area.row_stride*row + area.channels*col + color] - mean[color];
        sum[color] += dev*dev;
      }
...
#define BOTTOM 1
     ...
     incorrectly and thus ignored. */
  {
    ...
    if (x3f_get_prop_entry(x3f, "CAMMODEL", &cammodel))
      if (!strcmp(cammodel, "SIGMA DP2"))  use[BOTTOM] = 0;
  }
     ...
     specified incorrectly and thus ignored. */
  {
    ...
    if (x3f_get_camf_unsigned(x3f, "CAMERAID", &cameraid))
      if (cameraid == X3F_CAMERAID_SDQH)  use[BOTTOM] = 0;
  }
...
static int preprocess_data(x3f_t *x3f, int fix_bad, char *wb, x3f_image_levels_t *ilevels)
{
  ...
  x3f_printf(DEBUG, "cameraid %d, ISO %f, version %x\n", cameraid, sensor_iso, H->version);
  double rates[3] = { 1.0, 1.0, 1.0 };
  if (cameraid == X3F_CAMERAID_SDQH && H->version == 0x40002) {/*Only Quattro H and Firmware 1.04*/
  	if (sensor_iso == 100.0) {
	  	rates[0] = 1.2; rates[1] = 1.2; rates[2] = 0.6;
	  }
	}

  for (color = 0; color < 3; color++)
    scale[color] = ((ilevels->white[color] - ilevels->black[color]) / (max_raw[color] - black_level[color])) * rates[color];
